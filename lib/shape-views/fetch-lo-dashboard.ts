...
import { loNameFilterVariants, loNamesMatch } from "@/lib/shape-api/lo-roster";
import { hydrateLpCriticalDatesFromRawJson } from "@/lib/lendingpad/lp-critical-dates";
...
export async function fetchLoDashboardLoans(
  ...
  while (loans.length < maxRows) {
    ...
    if (options.assignedLoUserId || options.assignedLoName) {
      ...
      if (options.assignedLoName?.trim()) {
        const nameVariants = loNameFilterVariants(options.assignedLoName);
        for (const variant of nameVariants) {
          // ilike = case-insensitive exact match — includes Zack Davis aliases for Zachary Davis
          orParts.push(`assigned_loan_officer_name.ilike.${variant.trim()}`);
        }
      }
  ...
  const loName = options.assignedLoName?.trim() ?? null;
  ...
  const scoped =
    loUserId || loName
      ? loans.slice(0, maxRows).filter((row) => {
          ...
          if (loName && loNamesMatch(row.assigned_loan_officer_name, loName)) return true;
          return false;
        })
      : loans.slice(0, maxRows);

  const merged = mergeLoDashboardLoanRows(scoped.filter(passesGlobalFilters));
  await hydrateLoDashboardCriticalDates(supabase, merged);
  return { loans: merged, error: null };
}

export async function hydrateLoDashboardCriticalDates(
  supabase: SupabaseClient,
  loans: LoDashboardLoanRow[],
): Promise<void> {
  const needsHydration = loans.filter(
    (loan) =>
      loan.lendingpad_loan_uuid &&
      (!loan.le_issued_at || !loan.intent_to_proceed_at || !loan.lp_processing_at),
  );
  if (needsHydration.length === 0) return;

  const ids = needsHydration.map((loan) => loan.id);
  const lpRawByLoanId: Record<string, unknown> = {};

  for (let i = 0; i < ids.length; i += 200) {
    const chunk = ids.slice(i, i + 200);
    const { data } = await supabase.from("rich_loan_data").select("loan_id,lp_raw_json").in("loan_id", chunk);
    for (const row of data ?? []) {
      lpRawByLoanId[row.loan_id as string] = row.lp_raw_json;
    }
  }

  for (const loan of needsHydration) {
    hydrateLpCriticalDatesFromRawJson(loan, lpRawByLoanId[loan.id]);
  }
}
